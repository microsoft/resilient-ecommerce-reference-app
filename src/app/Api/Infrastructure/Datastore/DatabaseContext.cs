...
{
    /// <summary>
    /// Database representation of the application, using the abstracted Entity Framework.
    /// </summary>
    public class DatabaseContext : DbContext
    {
        private const int NUM_CONCERTS = 1000;
        ...
        private const int SQL_ERROR_TRACKING_ALREADY_ENABLED_FOR_DB = 5088;
        private const int SQL_ERROR_TRACKING_ALREADY_ENABLED_FOR_TABLE = 4996;
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<Concert>()
                .HasIndex(concert => new { concert.StartTime });
            modelBuilder.Entity<Ticket>()
                .HasIndex(ticket => new { ticket.UserId });
            modelBuilder.Entity<Order>()
                .HasIndex(order => new { order.UserId });
        }

        /// <summary>
        /// Initializes the database. Also initializes the 'Concerts' table with random concert data.
        /// <param name="numConcerts">the number of random concerts to add to the database when initializing.</param>
        /// </summary>
        public void Initialize()
        {
            ...
            if (Concerts.Any())
            {
                // DB already initialized. Nothing to do
                return;
            }
        ...
        private void EnableChangeTracking()
        {
            try
            {
                ...
                // Raw SQL as we can't parameterize the table name
                Database.ExecuteSqlRaw($"ALTER DATABASE [{databaseName}] SET CHANGE_TRACKING = ON (CHANGE_RETENTION = 2 DAYS, AUTO_CLEANUP = ON)");
            }
            catch (Microsoft.Data.SqlClient.SqlException exc)
            {
                if (exc.Number != SQL_ERROR_TRACKING_ALREADY_ENABLED_FOR_DB)
                {
                    // If the error thrown is 'change tracking is already enabled for the database', ignore
                    throw;
                }
            }
            ...
            try
            {
                ...
                // Raw SQL as we can't parameterize the table name
                Database.ExecuteSqlRaw($"ALTER TABLE [{concertsTableName}] ENABLE CHANGE_TRACKING WITH (TRACK_COLUMNS_UPDATED = ON)");
            }
            catch (Microsoft.Data.SqlClient.SqlException exc)
            {
                if (exc.Number != SQL_ERROR_TRACKING_ALREADY_ENABLED_FOR_TABLE)
                {
                    // If the error thrown is 'change tracking is already enabled for the table', ignore
                    throw;
                }
            }
        }
